...
activity_table = ActivityTableDouble.new
server_tables = { "table_activity" => activity_table }
activities = users.to_h do |user|
  [user, TableActivityRepository.new(server_tables: server_tables, transport: transports.fetch(user))]
end
lobbies = users.to_h do |user|
  [user, LobbyRepository.new(ProgramDouble.new(broker.endpoint(user)), transport: transports.fetch(user), server_tables: {}, activity_repository: activities.fetch(user))]
end
games = users.to_h do |user|
  [user, GameRepository.new(ProgramDouble.new(broker.endpoint(user)), transport: transports.fetch(user), server_tables: {})]
end
...
puts "Native LiveSessions store tests passed: discovery, membership, invitations, room state, game stack, chat and cleanup"
