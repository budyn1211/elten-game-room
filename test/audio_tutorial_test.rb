...
assert(entries.map(&:asset) == %w[audio_ball_up audio_ball_left audio_ball_down audio_ball_prepare],
  'Audio Ball tutorial does not cover its three shots and preparation in order')
assert(entries.map(&:label) == ['Ball sound: Up arrow or W', 'Ball sound: Left arrow or D',
  'Ball sound: Down arrow or S', 'Preparing the ball: Right arrow or A'], 'Tutorial labels do not explain the controls')
