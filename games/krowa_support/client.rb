...
module GameRoomGames
  ...
  class KrowaClient
    def initialize(program, game, server_tables: nil, transport: nil)
      @program, @game = program, game
      @server_tables = server_tables
      @private_reveal = KrowaPrivateReveal.new(game, transport)
    ...
    def start
      ...
      Log.warning("Krowa services: #{error.class}") if defined?(Log)
      alert(_("Could not load Krowa settings. Please try again."))
      false
    end
    ...
    def refresh_due?; @private_reveal.due?; end

    def tick
      @audio.update(@music_asset) if @music_asset
    end
    ...
    def action(selection, replay, viewer)
      if selection["kind"] == "command" && selection["action"] == "krowa_definition" &&
          replay.state[:surrender_words].keys.any? { |name| name.to_s.casecmp(viewer).zero? }
        word = @private_reveal.word || replay.state[:solution]
        word ? definition_dialog(word) : alert(_("Waiting for the private solution."))
        return true
      end
    ...
    def after_events(replay, viewer, context:)
      record_daily_completion(replay, viewer)
      begin
        word = if replay.state[:options]["variant"] == "race"
          EltenAPI::Tasks.run(title: _("Receiving the private solution"), cancellable: true, show_after: 2) do
            @private_reveal.update(replay, viewer, context)
          end
        end
        identity = [replay.state[:commitment], word]
        if word && !@shown_words[identity]
          @shown_words[identity] = true
          speak(_("Solution: %{word}.") % {word: word}, stop: false, break_sequence: false)
        end
      rescue StandardError => error
        Log.warning("Krowa private solution delivery: #{error.class}") if defined?(Log)
      end
