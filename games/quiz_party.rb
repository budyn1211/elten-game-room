...
module GameRoomGames
  class QuizParty < Base
    ...
    def rule_sections
      [
        rule_section(
          :goal,
          _("Goal"),
          _("Answer general knowledge questions faster and more accurately than the other players and be the first to reach the target score.")
        ),
        rule_section(
          :setup,
          _("Setup"),
      ...
      events.each do |event|
        ...
        when "answer_commit"
          if state[:phase] == :answering && includes_player?(players, actor) &&
            ...
            state[:commitments][canonical_player(players, actor)] = value
            accepted_event = true
          end
        when "answers_closed"
    ...
    def legal_actions(replay, actor, context: nil)
      ...
      when :revealing
        return [] if !player_hash_key?(state[:commitments], actor) || player_hash_key?(state[:reveals], actor)
    ...
    def reveal_plan(state, actor, context)
      ...
      [:ok, ActionPlan.new(events: [
        EventCommand.new(action: "answer_nonce", value: envelope.nonce),
        EventCommand.new(action: "answer_pick", value: envelope.payload.fetch("answer", "").to_s)
      ])]
    end
    ...
    def accept_reveal_part(state, players, actor, part, value)
      ...
      return false if part == :answer && !/\A[0-9]{1,2}\z/.match?(value)
