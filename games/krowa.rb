...
module GameRoomGames
  ...
  class Krowa < Base
    ...
    def build_client(program, **services)
      ...
      KrowaClient.new(program, self, **services)
    end

    def build_start_guard(program, user:, **services)
      require_relative "krowa_support/daily_access"
      KrowaDailyAccess.new(program, user: user, server_tables: services[:server_tables])
    end

    def build_leaderboard_client(program, **services)
      require_relative "krowa_support/leaderboards"
      KrowaLeaderboardClient.new(program, self, server_tables: services[:server_tables])
    end
    ...
    def save_game_error(replay)
      ...
      return _("Wait until the current word and all attempts have been checked before saving.") unless replay.state[:phase] == :active && replay.state[:pending].empty?
    ...
    def bot_reward(replay, player)
      return super unless replay.finished? && !tower?(replay.state)
      return 0.0 if replay.state[:winners].empty?
      replay.state[:winners].any? { |winner| same_user?(winner, player) } ? 1.0 : -1.0
    end
    ...
    def action_for(selection, replay, actor, context: nil)
      ...
      if kind == "question" && %w[submit add_word].include?(action)
        ...
        if adding && (!known_word?(state, word) || action == "add_word")
    ...
    def choice(key, label, default, values, visible_if: nil)
    ...
    def apply_event(state, event, actor, event_id, history)
      ...
      when "krowa_vocab"
        ...
        return false unless state[:phase] == :active && valid_added_word?(value)
