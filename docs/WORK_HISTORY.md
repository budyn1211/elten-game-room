# Historia prac nad Game Roomem

## 25 września 2026 — dalsze czternaście punktów utrzymywalności

Wdrożenie: [MAINTAINABILITY_FOLLOWUP.md](MAINTAINABILITY_FOLLOWUP.md).
Zakres nie obejmował zmiany reguł, siły botów, danych, tłumaczeń i nagrań.
Pozostawiono zabezpieczenia sesji oraz oddzielne wykonawce realtime/turowe.
Celowane próby: ostatnie wyniki 253 unikalnych scenariuszy poprawne, cztery
agregaty binarne oddzielnie; nie pełny przebieg. Pierwsze niepowodzenia
i kontrolne baseline zachowane. Składnia 245 zmienionych/dodanych Ruby poprawna.

Żywe próby na trzech kontach: 99, Scrabble, UNO, Makao, Remik z botem,
Audio Ball oraz Pong single/debel z botami. 182 porównania, również zamiany,
gospodarz-obserwator, powroty, fokus/szkic, kanały i odtwarzanie zapowiedzi.
To wejścia kontrolek i rzeczywiste API/relay na jednym komputerze, nie test
fizycznej klawiatury, odsłuch ani wszystkie możliwe pełne mecze.
Osobno 11 cykli pustych stołów potwierdziło limit retencji ośmiu oraz próba
odczytu prawdziwych tabel potwierdziła cache i unieważnianie bez zmian danych.
Nieukończone podejścia pomocników opisano oddzielnie.

Własne stoły zamknięte, sondy usunięte. Nowy kod pozostał w pamięci trzech
klientów na polecenie użytkownika. Bez instalacji, restartów hostów, profili,
zmian ochrony/schematu/quota, paczki/podpisu, wersji/changelogu lub publikacji.
Prywatne materiały: diagnostics/maintainability-followup-2026-09-25 katalogu
roboczego nadrzędnego, w tym zestawienia verification i potwierdzenia FINAL.

## Wcześniejsza chronologia

To zachowana chronologia przeniesiona z AGENTS.md 25 września 2026.
Wpisy opisują kolejne stany, dawne zgody i wyniki; nie są bieżącym zleceniem
budowania, wdrażania ani cofania zmian. Aktualny kontrakt jest w ../AGENTS.md.
Ścieżki w zachowanych wpisach odnoszą się do katalogu repozytorium, tak jak
przed przeniesieniem. Prywatne raporty poza repo mogą nie istnieć u innych osób.

## Dostęp do ustawień i zastępstwo realtime — 25 września 2026

Nie uzależniać lokalnych Ustawień od dostępu do tabel subskrypcji. Przy
odmowie/nieudanym odczycie pokazać tylko w tym miejscu objaśnienie; nie
zapisywać pustych subskrypcji za nieznaną listę. Przy częściowym błędzie
zapisu zachować lokalne zmiany, jawnie wskazując nieudany zapis serwerowy.
Regresje: settings_table_access_test oraz binarne PL/EN/fallback w
general_settings_discovery_encoding_test. Żywa kopia stamp_required
potwierdziła dostęp do wszystkich sześciu kategorii; anulowano bez zapisu.

Realtime bez SessionRunner musi wybudzić zewnętrzną politykę zastępstwa
także po samej zmianie członkostwa, bez nowego ruchu/epoki. GameScreen ma
transport w game_services[:transport], NIE w @transport. Testuj pełne run
do granicy zapisu i zwykłe wyjście z normalnego ekranu; ręczne zapisanie
zastępstwa w teście nie weryfikuje automatycznej reakcji na odejście.
Nie dodawaj metod do nil lub dodatkowych pól tylko po to, by atrapa
spełniła założenie produkcji. Sprawdzać także brak zastępstwa dla obecnej
osoby, obserwatora, przy nieudanym odczycie, po końcu/freeze, w grze bez botów
i przy istniejącym SessionRunner. Nie dodano pollingu ani nowej polityki
zastępowania po utracie Communications/ukryciu okna.

Pierwsza częściowa poprawka nie przeszła żywej próby i ujawniła drugi brak
transportu. Końcowe 22/22 celowane skrypty oraz trzy nowe próby UI/API:
gość Audio Ball/Pong, gospodarz Audio Ball. Bot zastępuje bez akcji drugiej
osoby, nowy gospodarz przejmuje grę, dalsze punkty działają. Obserwator może
wrócić i wyjść bez kolejnego bota; szkic/fokus zachowane. Jeden PC/łącze,
wejście handlerami, nie pełne partie/odsłuch lub gwarancja wszystkich awarii.
Stare fixture ustawień poprawiono na wybór kategorii po nazwie, nie indeksie.
Kontrolna mutacja w osobnym procesie potwierdza wykrywanie brakującego
transportu przez run, nie tylko zielony test nowego kodu.

Poprawki i trzy nowe tłumaczenia wczytane do main/20496, test/22920,
test-3/24268. Main zachowuje swój stół Audio Balla z botem; jednorazowe
wybudzenie starej ramki po hotloadzie opisane osobno od prób naturalnych.
Testowe kopie Scene_Main, sondy/własne stoły usunięte. Bez paczki, restartu,
pełnego reloadu, wersji/changelogu/GitHuba lub zmian ochrony/profili/serwera.
Raport prywatny ../diagnostics/settings-departure-304/README.md;
docs/ELTEN_3_0_4_TABLES_PLAN.md uaktualniony. Starsze poprawki zachować.

## Ponowne dołączenie po zastąpieniu botem — 25 września 2026

Na istniejącym stole użytkownika 99 odtworzono poprawne członkostwo
obserwatora z równoczesnym zamknięciem jego UI. Po leave pozostawał stary
sygnał closed w kolejce transportu. Guard callbacku starego Session nie
obejmował sygnałów już zakolejkowanych przed nowym dołączeniem. Zachować
oba zabezpieczenia: consume_recovery odrzuca :closed, jeśli lokalny
active_membership? potwierdza aktualne otwarte członkostwo. Bez I/O,
reconnectu, nowego pollingu lub wskrzeszania feedu starego wykonawcy.
Prawdziwe zamknięcie (także po eviction cache) i błędy sieciowe przechodzą.

Atrapa Session#leave musi emitować on_closed tak jak host; jej brak
maskował wcześniejsze reprodukcje. 19/19 celowanych skryptów, bez pełnego
runnera. Wczesne/spóźnione zamknięcie, trzy powroty w poczekalni/partii,
role po zastąpieniu, prawdziwe zamknięcie i brak nowego I/O. Na żywym stole
bez poprawki odtworzony błąd; potem trzy powroty z sondą i czwarty po jej
usunięciu poprawne. Stan gry/gospodarz/bot nie zmienione, zero ruchów
testowych; menu handlerami, potwierdzenia klawiaturą okna testowego.

Dwie metody poprawki pozostają w pamięci main/20496, test/22920,
test-3/24268, bez reloadu/restartu lub cofania poprzednich zmian 3.0.4.
Main/test nadal na stole, papiertestowy obserwatorem; test-3 Scene_Main.
Bez paczki, publikacji, wersji/changelogu, profili lub serwera.
Prywatny raport ../diagnostics/participant-rejoin-304/; opis projektu
docs/ELTEN_3_0_4_TABLES_PLAN.md. Nowa bezpośrednia reprodukcja uzupełnia
wcześniejsze niepotwierdzone podejrzenie poniżej, nie fałszuje tamtej próby.

## Wybrana osoba, 99 i gry bez botów — 25 września 2026

Aktualne Ctrl+Shift+R działa tylko na wybranym graczu/bocie na liście
użytkowników, od razu wybiera zastępstwo. Kandydaci: obecni ludzie bez
miejsca w partii, ewentualnie nowy nazwany bot w obsługującej go grze.
Nie przywracać pierwszej listy źródeł, zamiany dwóch grających osób ani
obserwatora jako źródła. Stare archiwa dawnych zamian nadal odtwarzać.
Ctrl+M pozostaje wyborem gospodarza. Menu/wybór rewalidować przed zapisem.

Callback on_closed nieaktualnego Session nie może zamknąć nowego widoku
ani wyczyścić oczekującego ruchu. Sprawdzać tożsamość bieżącego obiektu,
ale przyjmować także prawdziwe zamknięcie po wcześniejszym usunięciu go
z cache przez active_session. Pierwsza zbyt ścisła wersja ujawniła tę
regresję na żywo; ostateczny warunek ma osobny test. Reproduktor lokalny
jest pewny, związek z konkretnym zgłoszeniem użytkownika w 99 nieudowodniony.

26/26 celowanych skryptów, bez pełnego runnera. Pokryto wszystkie cztery
gry bez botów: Scrabble i Taboo z dalszą grą po zmianach; Państwa-miasta
z kopertami/powrotem/anulowaniem przez nowego gospodarza; Krowa Wyścig/Wieża
z powrotem gościa i dalszą oceną. Reguły Dziennej Krowy i blokady prywatnych
faz zachowane. Brak bota oznacza puste ludzkie miejsce, nie fikcyjnego bota
ani nową automatyczną karę za odejście. Obowiązują limity/fazy danej gry.

Trzy końcowe żywe próby: 17 porównań w 99, 14 w Scrabble z obserwatorem,
10 w Scrabble trzech graczy. Okna/role/ręce, szkic czatu, fokus historii,
wyjście/powrót, sukcesja i ostatni członek. To 41 porównań, nie pełne mecze.
Taboo/Krowa/Państwa-miasta lokalnie; nie nowe żywe partie. Wcześniejsze
niepełne próby/błędy pomocników osobno w raporcie. Jeden PC/łącze,
handlery natywnych kontrolek, nie fizyczna klawiatura lub odsłuch.

Prywatny raport ../diagnostics/participant-focus-304/, aktualny opis
docs/ELTEN_3_0_4_TABLES_PLAN.md. Końcowy reload usuwa sondy, pozostawia
217 źródeł i MO w main/20496, test/22920, test-3/24268. Nie przywracać
starszego kodu. Bez wersji/changelogu/paczki/podpisu/GitHuba, restartów,
instalacji, profili, zmian schematów lub quota.

## Zamiany na żywych stołach: poprawka UI — 25 września 2026

Na trzech rzeczywistych kontach odtworzono brak odświeżania po fizycznej
zamianie: wykonawca miał poprawne osoby, ale goście widzieli poprzednią
rękę/turę albo usuniętego bota aż do następnego ruchu. table_changed nie
oznacza wyłącznie zmiany listy osób. W GameScreen ścieżka room_refresh
sprawdza teraz także istniejącą projekcję sesji i wychodzi do odtworzenia
po zmianie epoki/właściciela. Bez nowego pollingu/wymuszonych odczytów.
Regresja room_interface_test obejmuje niezmienioną rewizję ruchów, nową
sesję oraz fokus/szkic czatu. Stary shared_history_clock_test otrzymał
brakującą tożsamość Alice w fixture, bez rozluźniania autoryzacji produkcji.
Polskie table master zmieniono na „gospodarz stołu”; kompilacja potwierdza
dokładnie jedną zmianę tłumaczenia, pozostałe wpisy zachowane.

21/21 celowanych skryptów i weryfikacja katalogu poprawne. Siedem prywatnych
stołów: reprodukcja oraz sześć prób poprawki — UNO, Makao, Spades drużynowy,
Pong single/debel, Audio Ball. Fizyczne role/ręce/drużyny, boty i ich
usunięcie po przywróceniu człowieka, odejście/powrót, nowy gospodarz,
sterowanie paletkami i dalsze trwałe punkty. 55 porównań, nie 55 pełnych
partii. Pierwszą próbkę przekazania uzupełnia późniejszy log tej samej
epoki bez nowego ruchu; początkowa próba punktu z autoodbijaniem i błędy
pomocników zachowane osobno. Rzeczywiste kontrolki, wejście handlerami,
API/relay na jednym PC; nie fizyczna klawiatura/odsłuch ani różne łącza.

Sondy i własne stoły usunięte. Aktualne 217 źródeł i MO pozostawione
w main/20496, test/22920, test-3/24268. Wszystkie Scene_Main, bez aktywnych
prób, nowa metoda i polska etykieta potwierdzone. Nie usuwać poprawki.
Bez paczki/instalacji/wersji/changelogu/GitHuba i zmian schematów/quota.
Raport prywatny ../diagnostics/participant-live-304/README.md i RESULT.json;
dokument projektu docs/ELTEN_3_0_4_TABLES_PLAN.md. Stare „tylko lokalne próby”
poniżej dotyczy poprzedniego etapu, nie bieżącego wyniku.

## Zamiana rzeczywistych uczestników — 24 września 2026

Użytkownik odrzucił „bot steruje człowiekiem”. W źródłach jest teraz
fizyczna zamiana osób: Ctrl+Shift+R, wybór źródła i zastępstwa; Ctrl+M
wybiera nowego gospodarza. Gracz↔obserwator zamienia role w obu kierunkach,
niezależnie od wskazania najpierw grającej czy obserwującej osoby.
Dwie grające osoby zamieniają miejsca/ręce/wynik/drużynę i pozostają
graczami. Nowy nazwany bot ma własną tożsamość, a człowiek obserwuje;
przy zastąpieniu bota człowiekiem bot znika ze stołu. Powrót konta nie
odbiera botowi miejsca bez decyzji gospodarza.

Niezmienny log i historyczne komunikaty zachowują poprzednich autorów.
Adapter participant_replay mapuje akcje według miejsc z danej chwili,
używając kontraktu odtwarzania gry; nie podmienia dowolnych tekstów stanu.
Zapis i odtworzenie obejmują historię zamian, także wielokrotnych i po
wcześniejszym odtworzeniu. Limity bajtów archiwum uwzględniają Unicode.
Naprawiono ujawnione w próbach: stary replay wykonawcy tuż po automatycznym
zastępstwie oraz zmianę pierwszego serwującego Ponga po nowym bocie.

69/69 celowanych skryptów poprawnych, bez SKIP i bez pełnego runnera.
Osobno kontrakty modeli 22 gier, dalsze ruchy zastępczego bota w 22,
drużyny, odebranie prawa do ruchu, zapis, kanały Ponga/Audio Balla i Unicode.
To próby LOKALNE, nie nowe partie na trzech kontach. Zachowano pierwsze
niepowodzenia roboczej wersji i pomocników, nie utożsamiać ich z końcowym
wynikiem. Wszystkie 4591 opublikowanych tłumaczeń zachowane, 31 dodanych
łącznie od HEAD; katalog skompilowany. Minimalne API 3.0.4, wersja/build
2.0.3.1/238 bez zmian. Podpisana 238 5997f31f… nietknięta.

Po kolejnym wyraźnym poleceniu użytkownika WGRANO korektę do pamięci
wszystkich trzech kopii: 20496/papierek, 22920/papiertestowy i
24268/papiertestowy1. Każde MCP potwierdziło ok:true, 217 źródeł i brak
stderr. Normalny unload/load z tymczasowym dostawcą bieżących źródeł,
PL/MO i dźwięków. NIE usuwać tej wersji; działa do przeładowania programu
lub restartu hosta. Instalacje na dysku bez zmian. Bez nowych prób,
restartów, zmian ustawień/serwera, paczki/changelogu lub GitHuba.
Potwierdzenia są w LOADED-FOR-USER.json obok raportu.
Dokument projektu: docs/ELTEN_3_0_4_TABLES_PLAN.md.
Prywatny raport: ../diagnostics/participant-replacement-304/README.md i TESTS.json.


## Nowy Game Room wczytany do trzech kopii — 24 września 2026

Na wyraźne polecenie użytkownika wczytano komplet bieżących źródeł
do pamięci main/PID20496/papierek, test/PID22920/papiertestowy
i test-3/PID24268/papiertestowy1. Wszystkie trzy wywołania MCP zakończone
ok:true, bez stderr. 216 plików Ruby, aktualny manifest API 3.0.4,
katalogi językowe i dźwięki. Normalny cykl unload/load hosta z jednorazowym
źródłem w pamięci; tymczasowa podmiana dostawcy źródeł od razu przywrócona.
Nie są to izolowane sondy — nowe funkcje są dostępne w zwykłym Game Roomie.
Działa do przeładowania aplikacji lub restartu ELTEN-a; zainstalowane
pliki/paczki niezmienione. NIE usuwać wczytanej wersji po zakończeniu pracy.

Użytkownik polecił „nie sprawdzaj, po prostu wczytuj”: nie wykonywano
dodatkowej kontroli stanu, testów ani prób partii. Potwierdzenia pochodzą
wyłącznie z samych wywołań wczytujących. Bez restartu klientów, nowej
paczki/podpisu/GitHuba lub zmian serwera. Helper i potwierdzenia:
../diagnostics/elten-304-points-1-2/{load-for-user.rb,LOADED-FOR-USER.json}.
Wcześniejszy wpis o niepozostawieniu kodu w pamięci opisuje zakończone testy.

## Pięć punktów ELTEN 3.0.4 ukończone w źródłach — 24 września 2026

Zakończono przekazywanie gospodarza i wyjście bez zamykania całego stołu,
sterowanie stałymi miejscami i zastępstwa botami, Ogólne/mowę/ding w tle,
prywatne zapisy konta 16 MiB oraz status/obsadę discovery. Minimalne API
manifestu i runtime to 3.0.4, discovery 7; wersja 2.0.3.1/build 238 bez zmian.
Nowe metadane właściciela uwierzytelniają historię kontrolerów. Nie
przeliczać dawnych zdarzeń według obecnego właściciela. Spóźnione akcje
starego kontrolera i polecenia poprzedniej partii są odrzucane.
Nie zastępować człowieka za samo ukrycie UI lub chwilowy brak aktualizacji;
tylko za potwierdzone odejście. Powrót nie wyrywa sterowania botowi.
Prywatne fazy mają jawne ograniczenia: Statki/Krowa podczas partii,
quiz/Państwa-miasta podczas odpowiedzi/ujawniania.
Pong/Audio Ball izolują kanały po zmianie, zachowują wynik i ponawiają
nieukończoną wymianę. Natywne członkostwo nie oznacza prawa do paletki:
odrzucać pozycje obserwatora/zastąpionego gracza. Makao/Monopoly zachowują
osobiste automatyczne czynności nowego gospodarza, niezależnie od miejsca.

66/66 celowanych skryptów po poprawkach; bez pełnego runnera. Kontrakty
23 gier i osobno przekazanie obserwatorowi/zastępstwo pierwszej osoby w 22.
59 plików Ruby poprawnych składniowo. Zachowano 4591 wcześniejszych
tłumaczeń i dodano 25; sprawdzono PO/MO, binarne źródła i natywne kontrolki.
Pierwsze niepowodzenia zachowane: dwa testy wskazały niewłaściwego hosta,
język był SKIP bez ELTEN_HOST_SOURCE; ponowiono z work/elten-test-client
w wersji 3.0.4. Wersja robocza ujawniła nadpisywanie paletki bota przez
zastąpionego człowieka oraz automaty Makao/Monopoly po zmianie gospodarza
— naprawione i objęte regresjami. Błędy pomocników oraz porównania
odtworzenia bez clock_offset udokumentowane, nie ukryte jako sukces.

Użyto trzeciej kopii jak polecono: test-3/PID24268/papiertestowy1/MCP37372,
ELTEN 3.0.4 RC 1 z aktualnego work/elten-test-client, osobne okno.
Pozostałe main/PID20496/papierek i test/PID22920/papiertestowy bez restartu.
Żywe próby na trzech kontach: Farkle, UNO, Pong, Audio Ball. Produkcyjne
API/relay/repo/runner/klienci w izolowanej przestrzeni, wejście pomocników,
ciche audio. Nie fizyczna klawiatura/odsłuch i nie trzy komputery.
Zapis UNO: nowy gospodarz, zamknięcie oryginału, odtworzenie w nowej sesji
i dalsze ruchy (łącznie 25 zdarzeń po odtworzeniu).
Usunięto tylko własne stoły/archiwum prób i sondy. Sprawdzono zwykłe wyjście
oraz natywne opuszczenie członkostwa; NIE zabicie procesu/wygaśnięcie dzierżawy.
Wszystkie trzy kopie: Scene_Main, podwątki 0, sondy i zasoby prób 0.
Nie pozostawiono nowego Game Roomu do normalnego grania; nie instalowano.

Szczegóły w docs/ELTEN_3_0_4_TABLES_PLAN.md, prywatny raport
../diagnostics/elten-304-points-1-2/{README.md,RESULT.json,TESTS-FINAL.json,
SOURCE.json,CLIENTS-FINAL.json,LIVE-*.json}. Limit 16 MiB bez zmian;
nie zmieniano tabel/ochrony, głównego hosta, changelogu, paczki lub GitHuba.
Podpisana 238 nadal 5997f31f… i NIE zawiera zmian 3.0.4. Przed wydaniem
zweryfikować finalny kontrakt hosta 3.0.4. PR #15 niewłączony.
Starsze wpisy o niewdrożonych punktach 1–2 niżej są historyczne.


## Testowy host ma już API 3.0.4 — 24 września 2026

Użytkownik zatwierdził aktualizację tylko testowego ELTEN-a i wykluczył
dodatkową starą kopię. Zaktualizowano istniejący ../work/elten-test-client
do dokładnych źródeł zainstalowanej 3.0.4 RC 1/build20260924001, zachowując
izolację okna i profil. PID22920/papiertestowy, MCP37374 odnowione,
Scene_Main, API3.0.4 z transfer_ownership/discovery refresh/private resources.
Główna PID20496/papierek bez zmian i restartu. Zgoda nie jest już blokadą.
Raport ../diagnostics/test-elten-304-upgrade/RESULT.json. Bez żywych prób
stołu, instalacji/hotloadu Game Roomu, buildu lub GitHuba. Punkty1–2 nadal
niewdrożone; dalsza integracja i testy wymagane. Wcześniejsze wpisy poniżej
o oczekiwaniu na zgodę są historyczne.

## ELTEN 3.0.4 — punkty 3–5 po dalszych testach lokalnych

Dopracowano Ogólne/mowę/ding, zapis na koncie i discovery; 31/31 celowanych
skryptów poprawnych, 27 plików składniowo. Sprawdzono odtworzenie przy nowym
stole i kolejny ruch, decyzje jednoczesne, brak sygnałów dla minionych tur
i nieaktualnej kolejki dźwięków, natywny tekst/kursor, PL/EN/fallback.
4591 wcześniejszych tłumaczeń zachowanych, osiem dodanych. Pełny stan:
docs/ELTEN_3_0_4_TABLES_PLAN.md, prywatny raport ../diagnostics/elten-304-points-3-5/.
Nie pełny runner ani żywe partie; źródła nadal wymagają integracji 3.0.4.
Punkty 1–2 NIEWDROŻONE. Nie podniesiono jeszcze API/wersji, nie budowano,
instalowano, hotloadowano, zmieniano profili/serwera ani wysyłano na GitHuba.
Podpisana 238 nietknięta. Zgoda na aktualizację tylko testowego hosta 3.0.3
jest nadal oczekiwana; bez niej nie aktualizować. Głównej kopii nie restartować.

## ELTEN 3.0.4 — wdrożenie rozpoczęte, limit 16 MB

Użytkownik zlecił wdrożenie pięciu punktów; wcześniejsze „tylko plan” poniżej
jest historyczne. Aktualny stan i pozostałe zadania zapisano na początku
`docs/ELTEN_3_0_4_TABLES_PLAN.md`. Kod punktów 3–5 jest roboczy, punkty 1–2
niewdrożone; nie przedstawiać całości jako ukończonej. Użytkownik zatwierdził
stałe automatyczne zastępstwa (bez opcji) oraz dźwięk `ding` dla własnej tury.
Po potwierdzeniu konta deweloperskiego i wyraźnej zgodzie limit prywatnych
plików zmieniono na **16 777 216 bajtów na konto**. Tabele, ich ochrona
i limit publiczny pozostały bez zmian. Dwa własne pliki próbne na kontach
papierek/papiertestowy sprawdzono i usunięto; prywatność między kontami
potwierdzona odmową odczytu 404. Żadnego nowego buildu/hotloadu/GitHuba.
Kopia testowa uruchomiona skryptem nadal ma 3.0.3, główna 3.0.4 RC 1;
zadano pytanie o zgodę na aktualizację tylko kopii testowej. Nie aktualizować
hosta ani głównej kopii samowolnie. Dokończyć implementację i testy przed wydaniem.

## Plan na ELTEN 3.0.4 — zbieranie wymagań, 24 września 2026

Bieżące propozycje zapisano w `docs/ELTEN_3_0_4_TABLES_PLAN.md`: przekazywanie
gospodarza i botów, zastępowanie graczy, dwa rodzaje pracy w tle ze wspólnymi
ustawieniami mowy/sygnału tury, zapisy na koncie i rzeczywisty status stołu
w discovery. Użytkownik wykluczył osobne przełączniki dla innego okna
ELTEN-a i dla przełączenia do innego programu. Oba przełączniki mają być
w kategorii „Ogólne” razem z wyborem języka interfejsu; bez osobnych
kategorii „Gra w tle” i „Język”. Zachować dotychczasowe działanie wyboru języka.
To plan do akceptacji, NIE zlecenie wdrożenia ani zmian serwera/paczki.
Domyślne wartości nowych opcji i zachowanie przy samych obserwatorach są
propozycjami. API 3.0.4 RC 1 i serwer potwierdzają jawne transfer_ownership
oraz aktualizację discovery; następstwo po nagłej awarii wymaga weryfikacji.
Nowego gospodarza nie wolno zastosować wstecz jako autora wszystkich
historycznych akcji. Chmura wymaga ponownego potwierdzenia magazynu;
dawne pomiary w SAVED_GAME_STORAGE_FEASIBILITY nie dowodzą możliwości 3.0.4.
Bez zmian produkcyjnych, nowych testów, instalacji, schematu lub GitHuba.
