...
Integration note (23 September 2026): the following is the original PR author's
verification record. PR #14's own delta has now been integrated into the main
checkout without reapplying its Audio Ball ancestry. Integration restores 61
newer local catalog entries. Current integration tests, source snapshots and
signed-package results are recorded separately in
`../diagnostics/pr-14-integration/`; the historical unsigned artifact below is
not the current test installer. See also `TEAMS_ROLES_AND_FOCUS.md` for the room
changes integrated alongside the language work.
